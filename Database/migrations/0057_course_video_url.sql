-- 0057_course_video_url.sql
--
-- Adds an optional VideoURL to edu.Course (uploaded video file or an
-- external link, e.g. YouTube/Vimeo) so admins can attach a preview video
-- to a course, played in a popup on both the admin editor and the public
-- course detail page. edu.Course_Get / edu.Course_List both SELECT c.* so
-- they pick the new column up automatically — only edu.Course_AddEdit needs
-- to be redefined to persist it. Otherwise identical to 0056.

IF COL_LENGTH('edu.Course', 'VideoURL') IS NULL
    ALTER TABLE edu.Course ADD VideoURL NVARCHAR(500) NULL
GO
