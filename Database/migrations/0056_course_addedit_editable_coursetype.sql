-- 0056_course_addedit_editable_coursetype.sql
--
-- CourseType (General/CSCA) was locked after creation because switching
-- CSCA -> General could orphan edu.CourseSubject rows and lose the
-- protected-delete guarantee CSCA courses used to have. Migration 0038
-- already dropped that protected-delete guarantee (CSCA courses are
-- deactivatable/deletable exactly like General ones now), so the only
-- remaining risk is leftover CourseSubject rows under a course that's no
-- longer CSCA. Allow CourseType to be edited, and when a course is
-- switched away from CSCA, delete its now-unused CourseSubject rows
-- (mirrors how every other child collection on this proc is wholesale-
-- replaced on save). Everything else in the proc is unchanged from 0031.
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            -- Switching away from CSCA leaves its exam-subject rows with no
            -- Subjects tab to manage them; drop them rather than leave them
            -- orphaned. Switching into/staying in CSCA is a no-op here.
            IF @CourseType <> 'CSCA'
            BEGIN
                DELETE FROM edu.CourseSubject WHERE CourseID = @CourseID;
            END
