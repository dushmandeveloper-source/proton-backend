...
{
    // Agent-facing sign-in. See Areas/Lecturer/Controllers/AccountController
    // — same shape, different portal and branding; shared plumbing is in
    // PortalSignIn.
    [Area("Agent")]
    public class AccountController : Controller
    {
        private const Portal ThisPortal = Portal.Agent;
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            ...
            if (issued)
            {
                var resetUrl = Url.Action("ResetPassword", "Account", new { area = "Agent", token }, Request.Scheme) ?? "#";
                var description = "We received a request to reset your Proton agent account password. This link expires in 30 minutes.";
